/**
 * Verify & Attest Handler
 *
 * Business logic for the verify-and-attest endpoint, extracted from route.ts
 * for testability. Follows the same pattern as controller-witness-handler.ts.
 *
 * Flow:
 * 1. Check for existing attestations (fast path)
 * 2. If missing → verify DID ownership (DNS/contract)
 * 3. If verified → write attestation to resolver
 */
...
export interface VerifyAndAttestParams {
  did: string;
...
export interface VerifyAndAttestResult {
  ok: boolean;
  status: 'ready' | 'failed';
  attestations: { present: string[]; missing: string[] };
  message?: string;
  error?: string;
  details?: any;
  method?: string;
...
export class VerifyAndAttestError {
  constructor(
...
function getActiveChain(): any {
  const activeChainEnv = process.env.NEXT_PUBLIC_ACTIVE_CHAIN || 'localhost';
  if (activeChainEnv === 'localhost') return localhost;
  if (activeChainEnv === 'omachain-testnet') return omachainTestnet;
  if (activeChainEnv === 'omachain-mainnet') return omachainMainnet;
  return null;
}
...
export async function verifyAndAttest(params: VerifyAndAttestParams): Promise<VerifyAndAttestResult> {
  const startTime = Date.now();
  debug('main', '=== NEW REQUEST ===');

  const { did, connectedAddress, requiredSchemas = ['oma3.ownership.v1'], txHash } = params;
  ...
  if (!did || typeof did !== 'string') {
    throw new VerifyAndAttestError(400, { ok: false, error: 'DID is required' });
  }

  if (!connectedAddress || typeof connectedAddress !== 'string') {
    throw new VerifyAndAttestError(400, { ok: false, error: 'Connected address is required' });
  }

  if (!isValidEthereumAddress(connectedAddress)) {
    throw new VerifyAndAttestError(400, {
      ok: false,
  ...
  if (!activeChain) {
    throw new VerifyAndAttestError(500, { ok: false, error: 'Invalid active chain' });
  }

  if (!activeChain?.contracts?.resolver) {
    throw new VerifyAndAttestError(500, { ok: false, error: 'Resolver not configured' });
  }
  ...
  const clientId = process.env.NEXT_PUBLIC_THIRDWEB_CLIENT_ID;
  if (!clientId) {
    throw new VerifyAndAttestError(500, { ok: false, error: 'Thirdweb client ID not configured' });
  }
  ...
  if (!verificationResult.success) {
    ...
    const elapsed = Date.now() - startTime;
    throw new VerifyAndAttestError(403, {
      ok: false,
      status: 'failed',
  ...
  for (const schema of missing) {
    try {
      const hash = await writeAttestation(
        did,
        connectedAddress,
        schema,
        activeChain.contracts.resolver,
      ...
      txHashes.push(hash);
      debug('main', `✅ Attestation written for schema ${schema}: ${hash}`);
    } catch (error) {
  ...
  if (writeErrors.length > 0 && txHashes.length === 0) {
    ...
    throw new VerifyAndAttestError(500, {
      ok: false,
      status: 'failed',
  ...
  return {
    ...
    status: 'ready',
    attestations: {
      present: [...present, ...missing.slice(0, txHashes.length)],
      missing: missing.slice(txHashes.length),
    },
    txHashes,
